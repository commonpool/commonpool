...
type RabbitCommandBus struct {
	...
	nameMap    map[string]commands.CommandHandler
...
func (r *RabbitCommandBus) RegisterHandler(handler commands.CommandHandler) error {
	if r.started {
	...
	for _, cmdType := range cmdTypes {
		...
		if _, ok := r.cmdMap[cmdType]; !ok {
			r.cmdMap[cmdType] = []commands.CommandHandler{}
		}
...
var _ commands.CommandBus = &RabbitCommandBus{}
