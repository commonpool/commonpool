...
func TestLockedListener(t *testing.T) {
	...
	for i := 0; i < listenerCount; i++ {
		...
		for j := 0; j < eventCount; j++ {
			events = append(events, evt("typ1", strconv.Itoa(j+i*eventCount)))
		}
	...
	runListener := func(i int, listener Listener, initialDelay time.Duration) {
		g.Go(func() error {
			...
			if err := listener.Listen(ctx, func(events []*eventstore.StreamEvent) error {
				t.Logf("entering listener %d", i)
				defer t.Logf(" exiting listener %d", i)
				for _, event := range events {
					callIds = append(callIds, event.EventID)
				}
				return nil
