...
func (h *Handler) GetRecentlyActiveSubscriptions(c echo.Context) error {
	...
	channelSubscriptions, err := h.chatService.GetSubscriptionsForUser(ctx, take, skip)
	if err != nil {
		return err
	}

	mappedSubscriptions, err := MapSubscriptions(ctx, h.chatService, channelSubscriptions)
	if err != nil {
		return err
	}

	return c.JSON(http.StatusOK, GetLatestSubscriptionsResponse{
